I need to develop a JavaScript program that validates the given Sudoku solution to be correct according to game rules as well as solves incomplete Sudoku puzzles and displays the solved results in a grid. The strategies like constraint propagation and backtracking can be used to achieve the desired functionality. In addition to it, the provided program code must be well commented, shouldn’t allow negative values and zeros in the grid. Please include usage code example to test the program with a variety of Sudoku puzzles, to verify correctness and provide a very brief explanation about the code. 
